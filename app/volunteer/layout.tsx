...
export default function VolunteerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && (!user || profile?.role !== 'volunteer')) {
      router.push('/login');
    }
  ...
  return (
    <div className="dash-layout">
      <Sidebar role="volunteer" userName={profile.name} userInitials={initials} />
      <Topbar title="Volunteer Dashboard" subtitle={`Welcome, ${profile.name}!`} />
